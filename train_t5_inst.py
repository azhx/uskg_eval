...
from utils.dataset import TokenizedDataset, TokenizedT5LlamaDataset
...
def main() -> None:
    ...
    import pickle
    if training_args.do_train and os.path.exists("./t5_inst_datasets.pkl"):
        with open("./t5_inst_datasets.pkl", "rb") as f:
        ...
        if training_args.local_rank <= 0:
            ...
            if training_args.do_train:
                with open(os.path.join("./t5_inst_datasets.pkl"), "wb") as f:
    ...
    model_tokenizer = AutoTokenizer.from_pretrained(args.bert.location, use_fast=True)
    ...
    train_dataset = TokenizedT5LlamaDataset(args, training_args, model_tokenizer,
                                    seq2seq_train_dataset) if seq2seq_train_dataset else None
    eval_dataset = TokenizedT5LlamaDataset(args, training_args, model_tokenizer,
                                    seq2seq_eval_dataset) if seq2seq_eval_dataset else None
    test_dataset = TokenizedT5LlamaDataset(args, training_args, model_tokenizer,
                                    seq2seq_test_dataset) if seq2seq_test_dataset else None

    import pickle
    pickle.dump(train_dataset, open("t5_inst_train_dataset.pkl", "wb"))
    print("saved train_dataset")
